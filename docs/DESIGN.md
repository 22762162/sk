...
| 2026-07-18 | 诚实核准+AI 问诊(PR #34,用户实测反馈驱动) | 反强行圆盘:dossier 摘要措辞改「盘上看不出就明说,严禁事后圆盘」;recalibrate 加马后炮铁律(判「符合」只认原推演原文,事后新盘理只能进 revision 标注待验);shichen 加区分力铁律(各候选都解释得通的大事不得作为依据);chat 加引盘诚实条款。AI 问诊:probe.md(3-5 个诊断性问题,答案能一锤定音,不问已知)+/api/probe/start+answer,回答以「问答对」入档案实录 |
| 2026-08-12 | 会诊诚实降级(PR #39,OpenAI 欠费事故驱动) | fail_closed 细化:第一轮单家失败不废整场——≥2 家在场则降级续跑(质证/裁判轮换均按在场者),payload.absent 如实标注缺席+前端黄条警示"互证力度弱于满编";<2 家仍如实报错。S1 按序补位。研究模式(拉丁方)保持严格满编。实测:假 OpenAI 钥匙 → 2 辩手+2 质证+裁判(anthropic)+白话全出,absent 标注正确 |
| 2026-08-11 | 生活化四件套+数据备份(PR #37) | ①择日:建除十二神+冲合刑害贵人禄刃逐日打分(luck.jianchu/day_score,确定性可对拍),AI只挑日子讲理由;②合婚:双人盘互参(夫妻宫/互补/流年同频);③命中率仪表盘:聚合旧预测jsonl+新App sqlite回访,按领域拆命中率(部分折半);④每日一盘:/api/app/today-reading 按日缓存(每天≤1调用),App今日卡内嵌;运势曲线SVG接入白话/校正两处流年;⑤备份:scripts/backup.sh+launchd每日03:30(dossier/appdata/records/manifests/.env→~/Documents/三鉴备份,留14份)。PR#36(Codex PWA)已验收合并 |
